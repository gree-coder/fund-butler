...
## 更新摘要
**所做更改**
- 移除腾讯财经数据源支持，用户界面不再显示腾讯财经作为数据源
- 优化数据源配置，仅保留天天基金、新浪财经和重仓股估算三个核心数据源
- 简化智能估价系统，减少数据源复杂度
- 更新前端数据源标签映射，移除腾讯财经相关配置
- 保持按基金代码移除功能的完整性
...
**重大更新**：自选股页面进行了全面重设计，增强了用户界面和交互功能，新增了智能估价系统和权重可视化展示，同时新增了按基金代码移除功能，提供更灵活的管理方式。**重要变更**：移除了对腾讯财经数据源的支持，简化了数据源配置，提升了系统的稳定性和性能。
...
DS2[SinaDataSource]
DS3[StockEstimateDataSource]
DS4[TencentDataSource]
end
subgraph "数据库"
DB1[watchlist表]
DB2[fund表]
...
+removeByCode(fundCode) Result~Void~
...
**类图来源**
- [WatchlistController.java:15-48](file://src/main/java/com/qoder/fund/controller/WatchlistController.java#L15-L48)
...
participant Service as WatchlistService
participant Mapper as WatchlistMapper
Client->>Controller : DELETE /api/watchlist/code/{fundCode}
Controller->>Service : removeByCode(fundCode)
Service->>Service : 构建查询条件
Service->>Mapper : delete(QueryWrapper)
Mapper-->>Service : 删除成功
Service-->>Controller : 返回成功
Controller-->>Client : 200 OK
```
...
**Section sources**
- [WatchlistController.java:15-48](file://src/main/java/com/qoder/fund/controller/WatchlistController.java#L15-L48)
- [WatchlistService.java:28-143](file://src/main/java/com/qoder/fund/service/WatchlistService.java#L28-L143)
...
- 性能指标：近1月、3月、1年收益展示
- **新增**：支持通过基金代码直接移除功能
- **更新**：移除了腾讯财经数据源选项，简化了数据源展示

**Section sources**
- [Watchlist 组件:78-205](file://fund-web/src/pages/Watchlist/index.tsx#L78-L205)
- [Watchlist API:29-45](file://fund-web/src/api/watchlist.ts#L29-L45)
...
| **/api/watchlist/code/{fundCode}** | DELETE | fundCode: string | void | **按基金代码删除** |
| /api/watchlist/check | GET | codes: string[] | string[] | 检查基金是否存在 |

**新增**：按基金代码删除接口，支持通过基金代码直接删除所有相关记录
...
**重要变更**：移除了腾讯财经数据源的权重计算，简化了权重分配逻辑
...
participant Service as WatchlistService
participant Mapper as WatchlistMapper
Client->>API : DELETE /api/watchlist/code/{fundCode}
API->>Controller : removeByCode(fundCode)
Controller->>Service : removeByCode(fundCode)
Service->>Service : 构建QueryWrapper
Service->>Mapper : delete(QueryWrapper)
Mapper->>Mapper : 执行删除操作
Mapper-->>Service : 删除成功
Service-->>Controller : 返回成功
Controller-->>API : 返回成功
API-->>Client : 200 OK
...
3. **批量查询**：优化多基金数据的批量获取
4. **优化按基金代码删除的查询性能**：确保fund_code字段有索引
...
5. **优化删除操作的用户体验**：添加删除确认和进度反馈
6. **简化数据源渲染**：移除腾讯财经数据源相关的UI元素
...
2. **权重缓存**：缓存权重计算结果
3. **准确度修正缓存**：缓存历史准确度数据
4. **减少数据源调用**：移除腾讯财经数据源后，减少不必要的API调用
...
- 加载骨架屏和响应式布局优化
- 更丰富的数据展示和交互功能
- **新增**：按基金代码移除功能，提供更灵活的管理方式
- **新增**：改进的API接口设计，支持多种删除方式
- **新增**：增强的数据处理能力，优化重复基金检测
- **重要变更**：移除了对腾讯财经数据源的支持，简化了数据源配置

**重要变更说明**：
系统已移除对腾讯财经数据源的支持，这是基于以下考虑：
1. **稳定性提升**：减少了第三方数据源的依赖，降低了系统复杂度
2. **性能优化**：减少了API调用次数，提升了整体响应速度
3. **维护成本降低**：简化了数据源管理，降低了维护工作量
4. **用户体验改善**：移除了冗余的数据源选项，提供了更清晰的界面
...
- **稳定的多数据源保障**：通过天天基金、新浪财经和重仓股估算确保数据的准确性和可靠性
- **良好的用户体验**：直观的界面设计和流畅的操作体验
- **可扩展性强**：模块化设计便于后续功能扩展
- **灵活的删除方式**：支持按ID和按基金代码两种删除方式
- **简化的数据源配置**：移除冗余数据源后，提供了更清晰的估价展示
