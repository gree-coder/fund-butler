...
## 更新摘要
**变更内容**
- **移除腾讯财经数据源**：腾讯财经API不再返回有效的基金实时估值数据，估值字段永远返回0，系统已完全移除TencentDataSource依赖
- **三源架构确立**：系统现为稳定的三数据源架构（东方财富、新浪财经、股票估值），权重算法已重新校准
- **智能降级策略优化**：getEstimateNav()方法实现新的三级降级策略：主数据源→备用数据源→股票估值兜底
- **熔断器配置调整**：移除tencent数据源配置，保留eastmoney、sina、stock的独立熔断器配置
- **权重算法重新校准**：基于新的三源架构重新计算数据源权重，提升估值准确性
- **前端数据源标签更新**：前端界面已更新数据源显示，移除腾讯数据源选项
...
**图表来源**
- [FundDataAggregator.java:45-54](file://src/main/java/com/qoder/fund/datasource/FundDataAggregator.java#L45-L54)
...
数据聚合器是系统的核心组件，负责协调三个数据源并提供统一的数据接口：
...
-buildSmartEstimate(fundCode, eastMoneyItem, sinaItem, stockItem, smartItem) void
...
- [StockEstimateDataSource.java:28-46](file://src/main/java/com/qoder/fund/datasource/StockEstimateDataSource.java#L28-L46)
...
**更新** 系统提供了三数据源估值获取功能，支持用户切换不同数据源，移除腾讯数据源依赖：
...
| source_key | VARCHAR(20) | 数据源标识：eastmoney/sina/stock |
...
**移除腾讯数据源配置：**
- **天天基金(eastmoney)**：失败阈值3，熔断60秒，半开最大尝试3次
...
**更新** 系统的getEstimateNav()方法实现了全新的三级数据源降级策略：
...
**章节来源**
- [FundDataAggregator.java:116-146](file://src/main/java/com/qoder/fund/datasource/FundDataAggregator.java#L116-L146)
...
多源基金估值系统是一个功能完整、架构清晰的现代化金融数据服务平台。系统通过三数据源聚合、智能估值算法和完善的缓存策略，为用户提供了准确、实时的基金数据服务。

### 系统优势

1. **三数据源支持**：集成三个权威数据源，确保数据的准确性和完整性
2. **智能估值算法**：实现三级降级的估值计算，提高估值的可靠性
...
22. **电路断路器优化**：移除tencent数据源配置，保留eastmoney、sina、stock的独立熔断器配置
...
- **电路断路器优化**：移除tencent数据源配置，保留eastmoney、sina、stock的独立熔断器配置
...
**更新** 本次更新重点反映了系统的重要变更：腾讯财经数据源已完全移除，系统现为稳定的三源架构（东方财富、新浪财经、股票估值），智能降级策略已优化为三级降级（主数据源→备用数据源→股票估值兜底），权重算法已重新校准，熔断器配置已调整，前端数据源标签已更新。这些变更显著提升了系统的稳定性、准确性和用户体验，特别是解决了腾讯财经API失效问题，确保了估值服务的连续性和可靠性。系统现已完全基于三个有效数据源运行，为用户提供更加稳定和准确的基金估值服务。