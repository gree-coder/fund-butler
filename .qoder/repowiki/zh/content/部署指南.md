...
**更新** 系统现已支持CLI和Web双模式部署，通过增强的Maven构建配置实现单一代码库的多模式运行能力。新增的双JAR打包配置（web和cli）允许应用程序灵活地作为Web服务和命令行界面工具部署。新增的cron作业安装脚本（scripts/install-cron.sh）提供了系统级的定时任务自动化部署能力，替代了原有的Spring @Scheduled注解方式，增强了任务调度的可靠性和自动化程度。
...
CliApp[FundCliApplication.java<br/>CLI模式启动类<br/>双模式支持]
Config[application.yml<br/>应用配置]
CliConfig[application-cli.yml<br/>CLI模式配置]
Schema[schema.sql<br/>数据库模式]
Data[data.sql<br/>初始化数据]
Cron[install-cron.sh<br/>系统级cron作业安装]
end
Root --> Main
Root --> CLI
Root --> Resources
...
**图表来源**
- [pom.xml:118-175](file://pom.xml#L118-L175)
...
**章节来源**
- [pom.xml:1-178](file://pom.xml#L1-L178)
...
系统提供两种运行模式的入口点，通过增强的Spring Boot插件配置实现：
...
FundCliApplication ..|> CommandLineRunner : "实现"
FundCliApplication ..|> ExitCodeGenerator : "实现"
SpringApplication --> FundApplication : "启动"
SpringApplication --> FundCliApplication : "启动"
```

**图表来源**
- [FundApplication.java:6-15](file://src/main/java/com/qoder/fund/FundApplication.java#L6-L15)
- [FundCliApplication.java:26-83](file://src/main/java/com/qoder/fund/cli/FundCliApplication.java#L26-L83)

### 增强的双JAR打包配置

项目使用Maven配置了双模式打包，通过明确的主类声明和classifier实现web和cli两种不同类型的JAR包：
...
基金管理系统采用前后端分离架构设计，支持CLI和Web双模式运行，通过增强的构建配置实现灵活部署：
...
subgraph "构建配置层"
Maven[Maven构建配置<br/>Spring Boot插件增强]
WebJar[Web模式JAR<br/>classifier: 无]
CliJar[Cli模式JAR<br/>classifier: cli]
MainClass[主类声明<br/>明确的启动类]
end
Web --> Core
Mobile --> Core
CLI --> Core
...
系统提供完整的命令行工具，支持多种操作命令，通过增强的构建配置实现灵活部署：
...
**图表来源**
- [FundCommand.java:17-29](file://src/main/java/com/qoder/fund/cli/FundCommand.java#L17-L29)
...
Cli[CLI配置<br/>application-cli.yml]
end
subgraph "配置优先级"
Override[属性覆盖]
Env[环境变量]
...
本部署指南提供了基金管理系统从开发到生产的完整部署方案。系统基于Spring Boot构建，前端使用React + Vite技术栈，现已支持CLI和Web双模式部署，通过增强的Maven构建配置实现单一代码库的多模式运行能力。

**更新** 关键新特性包括：
- 支持CLI和Web双模式部署
- 增强的双JAR打包配置（web和cli），包括明确的主类声明
- CLI命令行工具完整功能
- Web模式和CLI模式的灵活切换
- 增强的容器化部署选项
- 新增系统级cron作业安装脚本
...
建议根据实际业务需求调整配置参数，并定期更新安全补丁和依赖版本。对于生产环境，强烈建议使用系统级cron作业安装脚本，以获得更好的任务调度可靠性和维护便利性。同时，利用增强的Maven构建配置确保双模式部署的一致性和可靠性。